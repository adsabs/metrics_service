...
allowed_types = ['basic','citations','histograms','indicators','timeseries']
    ...
    def post(self):
        ...
        if 'bibcodes' in request.json:
            ...
            if len(bibcodes) > current_app.config.get('METRICS_MAX_SUBMITTED'):
                return {'Error': 'Unable to get results!', 'Error Info': 'No results: number of submitted bibcodes exceeds maximum number'}, 200
            elif len(bibcodes) == 0:
                return {'Error': 'Unable to get results!', 'Error Info':'No bibcodes found in POST body'}, 200
        elif 'query' in request.json:
            query = request.json['query']
        else:
            return {'Error': 'Unable to get results!', 'Error Info': 'Nothing supplied to calculate metrics on!'}, 200 
        results = generate_metrics(bibcodes=bibcodes,query=query,tori=include_tori,types=types,histograms=histograms)
