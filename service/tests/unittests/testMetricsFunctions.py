...
class TestSelfCitationFunction(TestCase):
  ...
  def test_get_selfcitations(self):
    ...
    data,selfcits,Ns,Ns_r,Nc,Nc_r = get_selfcitations([1,2,3],testset)
    # The 'data' returned is upposed to be a list of MetricsModel objects
    self.assertEqual(isinstance(data, list),True)
    self.assertTrue(False not in [x.__class__.__name__ == 'MetricsModel' for x in data])
    # The 'selfcits' returned is supposed to be a list to determine self-citation data
    # it is a list of tuples with a 'set' as first element and a boolean as second
    self.assertTrue(False not in [isinstance(x[0], set) and isinstance(x[1], bool) for x in selfcits])
    # Get the actual self-citations
    selfs = list(itertools.chain(*[x[0] for x in selfcits if len(x[0]) > 0]))
    self.assertEqual(sorted(selfs), sorted(expected_results['self-citations']))
    # Now check the number of self-citations
    self.assertEqual(Ns, expected_results['citation stats']['number of self-citations'])
    # and the refereed number of self-citations
    self.assertEqual(Ns_r, expected_results['citation stats refereed']['number of self-citations'])
    # Finally there is the number of citing papers
    # Note: this is higher than the actual number for the 'test set' because of the
    # nature of the mock data (the citations of the citations are also in the data)
    self.assertEqual(Nc, expected_results['citation stats']['number of citing papers'])
    # and the refereed variant
    self.assertEqual(Nc_r, expected_results['citation stats refereed']['number of citing papers'])
...
class TestToriFunction(TestCase):
  ...
  def test_get_tori(self):
    ...
    self.assertAlmostEqual(tori_ref, expected_results['indicators refereed']['tori'])
    # The riq follows from normalizing by date range
    self.assertAlmostEqual(riq_ref, int(1000.0*sqrt(tori_ref)/float(yrange)))
...
class TestUsageHistogram(TestCase):
  ...
  def test_usage_histograms(self):
    ...
    expected = dict([(year,expected_results['basic stats']['number of papers']) for year in range(1996, current_year+1)])
    ...
    expected = dict([(year,expected_results['basic stats refereed']['number of papers']) for year in range(1996, current_year+1)])
...
class TestCitationHistogram(TestCase):
  ...
  def test_citation_histograms(self):
    ...
    histograms = ['refereed to refereed', 'refereed to nonrefereed',
                  'nonrefereed to refereed', 'nonrefereed to nonrefereed',
                  'refereed to refereed normalized', 'refereed to nonrefereed normalized',
                  'nonrefereed to refereed normalized', 'nonrefereed to nonrefereed normalized']
    for histogram in histograms:
        # Get the expected values
