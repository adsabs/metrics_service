...
# 6. None of the records have refereed citations: citation histograms for refereed
#    citations should be returned with all zero entries
...
def get_test_data(bibcodes=None, htest=False, no_cits=False, no_refcits=False, no_usage=False, single=False, tori_test=False):
    # 'bibcode'   : if a list with bibcodes is specified, only retrieve data
    #               for these bibcodes
    # 'htest'     : flag for a specific test for the h index: assign an very high
    #               artificial number of citations to each paper to test that
    #               the h index will equal the number of records in this case
    # 'no_cits'   : flag for a specific test for the h/g index: make the number of
    ...
    # 'single'    : flag for special case of single bibcode without reads and citations
    # 'tori_test' : flag for the special case where all citations will be self-citations
    ...
    for dfile in datafiles:
        ...
        if htest:
            ...
            data['rn_citation_data'] = [{'bibcode':data['bibcode'],'auth_norm':1,'ref_norm':1}]
        if not no_usage:
            r = MetricsModel(
                id=data['id'],
...
class TestHirschExtreme(TestCase):
  '''Check if h index is the number of records when for all records: Ncits > Nrecs'''
  def create_app(self):
  ...
  def test_get_h_extreme(self):
    '''Test h index for only records where Ncits > Nrecs: h should equal Nrecs'''
...
class TestNoUsage(TestCase):
  ...
  def test_no_usage(self):
    ...
    histograms = ['all reads','refereed reads','all reads normalized','refereed reads normalized']
    ...
    histograms = ['all downloads','refereed downloads','all downloads normalized','refereed downloads normalized']
    for histogram in histograms:
        self.assertEqual(hist[histogram], expected)
...
class TestNoRefereedCitations(TestCase):
  ...
  def test_no_refereed_citations(self):
    '''Test getting citation histograms when there are no refereed citations'''
    from utils.metrics import get_citation_histograms
    years = [int(b[:4]) for b in testset]
    # Expected histogram (for all)
    expected = {year:0 for year in range(min(years),current_year+1)}
...
class TestNoTimeSeries(TestCase):
  ...
  def create_app(self):
    ...
    exe = db.session.execute
    mtr = db.metrics.execute
    exe.return_value = get_test_data(bibcodes=testset, no_cits=True, no_usage=True)
    mtr.return_value = get_test_data(bibcodes=testset, no_cits=True, no_usage=True)
    return app_
...
class TestMetricsSingleBibcodeNoUsageCitations(TestCase):
  ...
  def test_get_metrics_single_invalid_bibcode(self):
    '''Test getting data for a single bibcode without usage and citations'''
