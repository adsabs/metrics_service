...
testset = ['1997ZGlGl..33..173H','1997BoLMe..85..475M','1997BoLMe..85...81M','2014bbmb.book..243K','2012opsa.book..253H']
...
# The mockdata to be returned by the Solr mock, which is supposed to return just 
# the bibcodes for our test set
mockdata = [
...
class TestBasicStatsBibcodes(TestCase):
  ...
  def test_get_basic_stats_bibcodes(self):
    ...
    self.assertTrue(r.json.keys(), [u'basic stats', u'skipped bibcodes', u'basic stats refereed'])
    ...
    self.assertAlmostEqual(bs['normalized paper count'],expected_results['basic stats']['normalized paper count']) 
    # With the reads set up as one per year, the total number of reads should
    # amount to the number of years since 1996 times the number of papers
    ...
    self.assertEqual(bs['total number of downloads'],Nentries*len(testset))
    ...
    self.assertEqual(bsr['number of papers'], expected_results['basic stats refereed']['number of papers'])
    # The normalized paper count
    self.assertAlmostEqual(bsr['normalized paper count'],expected_results['basic stats refereed']['normalized paper count'])
  ...
  def test_get_basic_stats_bibcodes_with_invalid(self):
    ...
    r = self.client.post(
          url_for('metrics.metrics'),
          content_type='application/json',
          data=json.dumps({'bibcodes':testset+['foo'], 'types':['basic']}))
    self.assertTrue(r.status_code == 200)
...
class TestBasicStatsQuery(TestCase):
  ...
  def test_get_basic_stats_query(self):
    ...
    # There should be no skipped bibcodes
    self.assertEqual(r.json['skipped bibcodes'], [])
    # We have already checked in the testMetricsFunctions unittests that the
    # correct bibcodes are returned and given to the metrics framework, so
    # the results returned are by definition the same and need not be tested
...
class TestCitationStatsBibcodes(TestCase):
  ...
  def test_get_citation_stats_bibcodes(self):
    '''Test getting just citation stats when valid bibcodes are submitted'''
    r = self.client.post(
          url_for('metrics.metrics'),
          content_type='application/json',
    ...
    self.assertTrue(r.status_code == 200)
    # Check that the right info is returned; nothing more, nothing less
    self.assertTrue(r.json.keys(), [u'citation stats', u'skipped bibcodes', u'citation stats refereed'])
    # There should be no skipped bibcodes
    self.assertEqual(r.json['skipped bibcodes'], [])
    ...
    for check in citation_checks:
        self.assertEqual(cs[check],expected_results['citation stats'][check])
    # and the same for the citation stats for refereed papers
    for check in citation_checks:
        self.assertEqual(csr[check],expected_results['citation stats refereed'][check])
...
class TestIndicatorsBibcodes(TestCase):
  ...
  def test_get_indicators_bibcodes(self):
    '''Test getting just the indicators when valid bibcodes are submitted'''
    r = self.client.post(
          url_for('metrics.metrics'),
          content_type='application/json',
    ...
    self.assertTrue(r.status_code == 200)
    # Check that the right info is returned; nothing more, nothing less
    self.assertTrue(r.json.keys(), [u'indicators', u'skipped bibcodes', u'indicators refereed'])
    # There should be no skipped bibcodes
    self.assertEqual(r.json['skipped bibcodes'], [])
    ...
    for indicator in indicators:
        self.assertEqual(indic[indicator],expected_results['indicators'][indicator])
    self.assertAlmostEqual(indic['tori'],expected_results['indicators']['tori'])
    ...
    for indicator in indicators:
        self.assertEqual(indic_ref[indicator],expected_results['indicators refereed'][indicator])
    self.assertAlmostEqual(indic_ref['tori'],expected_results['indicators refereed']['tori'])
...
class TestPublicationHistogramsBibcodes(TestCase):
  ...
  def test_get_publication_histograms_bibcodes(self):
    '''Test getting just publication histograms when valid bibcodes are submitted'''
    r = self.client.post(
          url_for('metrics.metrics'),
          content_type='application/json',
          data=json.dumps({'bibcodes':testset, 'types':['histograms'], 'histograms':['publications']}))
    self.assertTrue(r.status_code == 200)
    # Check that the right info is returned; nothing more, nothing less
    ...
    # Only compare the non-zero entries (the returned dictionary will differ from
    # year to year, since next year there will be an additional zero)
    histograms = ['all publications', 'all publications normalized',
                  'refereed publications', 'refereed publications normalized']
    for histogram in histograms:
        # Get the expected values
        expected = expected_results['histograms']['publications'][histogram]
        # Get the non-zero entries for the histogram we just generated
        nonzero = dict([(year,freq) for year,freq in hist[histogram].items() if freq <> 0])
        # and compare
...
class TestUsageHistogramsBibcodes(TestCase):
  ...
  def test_get_reads_histograms_bibcodes(self):
    '''Test getting just usage histograms when valid bibcodes are submitted'''
    r = self.client.post(
          url_for('metrics.metrics'),
          content_type='application/json',
          data=json.dumps({'bibcodes':testset, 'types':['histograms'], 'histograms':['reads']}))
    self.assertTrue(r.status_code == 200)
    # Check that the right info is returned; nothing more, nothing less
    self.assertTrue(r.json.keys(), [u'histograms', u'skipped bibcodes'])
    self.assertTrue(r.json['histograms'].keys(), ['reads'])
    self.assertTrue(r.json['histograms']['reads'].keys(), ['all reads','refereed reads'])
    hist = r.json['histograms']['reads']
    # Every entry in the histogram for all papers should, by design,
    # equal the number of papers in the test set, from 1996 up to the current year
    current_year = datetime.now().year
    expected = dict([(str(year),expected_results['basic stats']['number of papers']) for year in range(1996, current_year+1)])
    self.assertEqual(hist['all reads'], expected)
    # and for the same reason, every entry in the histogram for refereed papers should
    # equal the number of refereed papers
    expected = dict([(str(year),expected_results['basic stats refereed']['number of papers']) for year in range(1996, current_year+1)])
    self.assertEqual(hist['refereed reads'], expected)
    # For the normalized histograms, each entry should equal the normalized paper count
    # Because we're dealing with a dictionary of floats, we do things slightly differently:
    ...
    self.assertAlmostEqual(hist['all reads normalized'].values()[0],expected_results['basic stats']['normalized paper count'])
    # Because the downloads have been constructed in the same way, we only need to
    # verify that the downloads histograms are the same as the reads ones
    r = self.client.post(
          url_for('metrics.metrics'),
          content_type='application/json',
          data=json.dumps({'bibcodes':testset, 'types':['histograms'], 'histograms':['downloads']}))
    self.assertTrue(r.status_code == 200)
    # Check that the right info is returned; nothing more, nothing less
    self.assertTrue(r.json.keys(), [u'histograms', u'skipped bibcodes'])
    self.assertTrue(r.json['histograms'].keys(), ['downloads'])
    self.assertTrue(r.json['histograms']['downloads'].keys(), ['all downloads','refereed downloads'])
    ...
    self.assertEqual(hist['all reads normalized'], dhist['all downloads normalized'])
    self.assertEqual(hist['refereed reads normalized'], dhist['refereed downloads normalized'])
...
class TestCitationHistogramsBibcodes(TestCase):
  ...
  def test_get_citation_histograms_bibcodes(self):
    '''Test getting just citation histograms when valid bibcodes are submitted'''
    r = self.client.post(
          url_for('metrics.metrics'),
          content_type='application/json',
          data=json.dumps({'bibcodes':testset, 'types':['histograms'], 'histograms':['citations']}))
    self.assertTrue(r.status_code == 200)
    # Check that the right info is returned; nothing more, nothing less
    self.assertTrue(r.json.keys(), [u'histograms', u'skipped bibcodes'])
    ...
    # Histogram types
    histograms = [u'refereed to nonrefereed', u'nonrefereed to nonrefereed', 
...
class TestAllHistogramsBibcodes(TestCase):
  ...
  def test_get_all_histograms_bibcodes(self):
    ...
    # Check that the right info is returned; nothing more, nothing less
    self.assertTrue(r.json.keys(), [u'histograms', u'skipped bibcodes'])
    self.assertTrue(r.json['histograms'].keys(), ['publications','usage','citations'])
...
class TestTimeSeriesBibcodes(TestCase):
  ...
  def test_get_timeseries_bibcodes(self):
    ...
    for indicator in indicators:
        ...
        expected = {k:v for k,v in expected_results['time series'][indicator].items()} 
...
class TestEverythingBibcodes(TestCase):
  ...
  def test_get_everything_bibcodes(self):
    '''Test getting everything when no specific metrics type is specified'''
    r = self.client.post(
          url_for('metrics.metrics'),
          content_type='application/json',
...
class TestMetricsSingleBibcode(TestCase):
  ...
  def test_get_metrics_single_bibcode(self):
    ...
    expected_keys = ['basic stats', 'histograms', 'skipped bibcodes', 'basic stats refereed']
    self.assertEqual(r.json.keys(), expected_keys)
