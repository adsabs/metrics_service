...
class TestConfig(TestCase):
  ...
  def test_config_values(self):
    ...
    required = ["METRICS_MAX_SUBMITTED","METRICS_CHUNK_SIZE","METRICS_MAX_HITS","METRICS_SOLRQUERY_URL","SQLALCHEMY_BINDS","DISCOVERER_PUBLISH_ENDPOINT","DISCOVERER_SELF_PUBLISH","METRICS_CLIENT"]
    ...
    if os.path.exists("%s/local_config.py" % PROJECT_HOME):
      self.assertTrue(self.app.config.get('METRICS_API_TOKEN',None) != None)
...
class TestMetricsModel(TestCase):
  ...
  def test_metrics_model(self):
    ...
    mc = [Column(Integer),Column(String),Column(Boolean),Column(postgresql.REAL),
          Column(postgresql.JSON),Column(postgresql.ARRAY(Integer)),
          Column(postgresql.ARRAY(Integer)),Column(postgresql.REAL),
          Column(Integer),Column(Integer),Column(Integer),
          Column(postgresql.ARRAY(String)),Column(postgresql.ARRAY(String)),
          Column(Integer),Column(postgresql.REAL),Column(DateTime)]

    expected = map(type, [x.type for x in mc])
    self.assertEqual([type(c.type) for c in MetricsModel.__table__.columns], expected)
...
class TestHelperFunctions(TestCase):
  ...
  def test_mock_query(self):
    ...
    expected_attribs = ['_sa_instance_state', 'author_num', 'bibcode', 'citation_num', 'citations', 'downloads', 'id', 'reads', 'refereed', 'refereed_citation_num', 'refereed_citations', 'rn_citation_data']
    # Quering the mock should return a list of MetricsModel instances
    resp = db.session.execute()
    self.assertEqual(sorted(resp[0].__dict__.keys()), sorted(expected_attribs))
    self.assertEqual(list(set([x.__class__.__name__ for x in resp])),['MetricsModel'])
    self.assertTrue(type(resp) == type([]))

  def test_bind_object(self):
    '''Check that the Bind object is what is expected'''
    b = Bind('metrics')
    self.assertEqual(b.__class__.__name__,'Bind')
    self.assertEqual(b.__dict__.has_key('bind'),True)
...
class TestPublicationDataRetrieval(TestCase):
  '''Check if the publication data retrieval function returns expected results'''
  def create_app(self):
    '''Create the wsgi application'''
    app_ = app.create_app()
    db.session = mock.Mock()
...
class TestCitationDataRetrieval(TestCase):
  '''Check if the citation data retrieval function returns expected results'''
  def create_app(self):
    '''Create the wsgi application'''
    app_ = app.create_app()
    db.session = mock.Mock()
...
class TestIndicatorDataRetrieval(TestCase):
  '''Check if the indicator data retrieval function returns expected results'''
  def create_app(self):
    '''Create the wsgi application'''
    app_ = app.create_app()
    db.session = mock.Mock()
