...
class TestNoRecordInfoFound(TestCase):
  ...
  def test_illegal_retrieval_method(self):
    '''No record info is found when an unsupported retrieval method is specified'''
    from utils.metrics import get_record_info
    data = get_record_info(other="foo")
    expected = {'Status Code': 200, 'Error Info': 'Unsupported metrics request', 'Error': 'Unable to get results!'}
...
class TestBadRequests(TestCase):
  '''Tests that no or too many submitted bibcodes result in the proper responses'''
  ...
  def testEmptyBibcodeListSubmitted(self):
    '''When an empty list of bibcodes is submitted an error should be returned'''
    r = self.client.post(
  ...
  def testNothingSubmitted(self):
    '''When no bibcodes nor a query is submitted an error should be returned'''
    r = self.client.post(
          url_for('metrics.metrics'),
          content_type='application/json',
          data=json.dumps({}))
